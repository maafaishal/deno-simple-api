...
export const getAllAuctions = async ({ request, response }: { request: any, response: any }) => {
  try {
    ...
    const result = await client.query(`SELECT * FROM auctions ${additional} ORDER BY created_date, auction_id ASC ${paging}`);
    const auctionsTemp = mapResponse(result);

    const dataCount = await client.query(`SELECT COUNT(*) FROM auctions ${additional}`);
...
export const getAuction = async ({ request, response }: { request: any, response: any }) => {
  try {
    ...
    const result = await client.query(`SELECT * FROM auctions WHERE auction_id = ${id}`);
    const auctionsTemp = mapResponse(result);
